...
if(isset($_REQUEST['enviar']) && $_REQUEST['enviar']=="Cadastrar"){
    if(mysql_query("INSERT INTO eventos (nome_evento, subtitulo, data, nome_local, descricao, programacao, participantes, inscricao, regulamento) VALUES ('$nome_evento', '$subtitulo', '$d_ini', '$nome_local', '$descricao', '$programacao', '$participantes', '$inscricao', '$regulamento')")){
    ...
    $id = $_REQUEST['id'];
    if(mysql_query("UPDATE eventos SET nome_evento='$nome_evento', subtitulo='$subtitulo', data='$d_ini', nome_local='$nome_local', descricao='$descricao', programacao='$programacao', participantes='$participantes', inscricao='$inscricao', regulamento='{$regulamento}' WHERE id = ".$id."")){
