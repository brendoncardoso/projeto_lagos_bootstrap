...
<?php if(isset($_REQUEST['method']) && $_REQUEST['method']=="exclui"){ ?>
    ...
    ?>
<?php } else if (isset($_REQUEST['method']) && $_REQUEST['method']== "listaAno") { ?>
    <?php
...
if(isset($_REQUEST['enviar']) && $_REQUEST['enviar']=="Cadastrar"){
    if(mysql_query("INSERT INTO compras (id_unidade, numero, proc_adm, licenciatorio, data_ini, data_fim, observacao) VALUES ({$unidade}, '{$numero}', '{$proc_adm}', '{$licitatorio}', '{$d_ini}', '{$d_fim}','{$observacao}');")){
        $id = mysql_insert_id();
        //UPLOAD DO EDITAL
        ...
        if (!empty($_FILES)) {
            ...
            $name = "c" . $id . "-" . str_replace(" ", "_", microtime()) . ".pdf";
        ...
        mysql_query("UPDATE compras SET edital = '{$dirSave}{$name}' WHERE id_compra = {$id}");
    ...
    $id = $_REQUEST['id'];
    if(mysql_query("UPDATE compras SET id_unidade='$unidade', numero='$numero', proc_adm='$proc_adm', licenciatorio='$licitatorio', data_ini='$d_ini', data_fim='$d_fim', status='$status', observacao='{$observacao}' WHERE id_compra = ".$id."")){
