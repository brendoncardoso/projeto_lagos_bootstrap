...
$sql_t = mysql_query("SELECT A.*, B.nome AS nome_unidade
                ...
                WHERE A.status = 1 AND A.tipo = 2                

                ORDER BY A.ano, B.nome, A.subtipo") or die(mysql_error());
        ...
        <script>
            ...
            $(function(){
                ...
                $("#novo").click(function(){
                    ...
                    window.location = 'relatorio_anual_form.php';
                    ...
                    if(confirm('Atenção, essa ação é irreversível, deseja realmente excluir esse relatório?')){
                        ...
                        $.ajax({
                            ...
                            data:{
                                ...
                                method:"excluir_relatorio"

                            },

                            
                            success:function(data){
                                window.location.href="relatorio_anual.php"
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="some uni<?php echo $row['id_unidade']; ?>_<?php echo $row['ano']; ?> rel<?php echo $row['id_execucao']; ?>">
                                            ...
                                            <td>
                                                ...
                                                if ($row['subtipo'] == 1){$row['nomesubtipo'] = "Relatório";}

                                                if ($row['subtipo'] == 2){$row['nomesubtipo'] = "Balancete";} 

                                                if ($row['subtipo'] == 3){$row['nomesubtipo'] = "Inventário";}

                                                echo $row['nomesubtipo'];?>

                                            </td>

                                            <td>

                                            <a href="rel_anual/<?php echo $row['id_unidade']."_".$row['tipo']."_".$row['subtipo']."_".$row['ano'].".pdf"; ?>" target="_blank">Visualizar arquivo</a></td>

                                            <td class="f_left">

                                                <a href="javascript:;" class="icon icon-excluir" title="Excluir" data-key="<?php echo $row['id_execucao'];?>" data-file="<?php echo $row['id_unidade']."_".$row['tipo']."_".$row['subtipo']."_".$row['ano'].".pdf";?>" data-del="rel<?php echo $row['id_execucao'];?>">&nbsp;</a>
